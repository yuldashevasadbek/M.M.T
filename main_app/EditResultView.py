...
class EditResultView(View):
    def get(self, request, *args, **kwargs):
        ...
        context = {
            'form': resultForm,
            'page_title': "O'quvchi natijasini tahrirlash"
        }
        return render(request, "staff_template/edit_student_result.html", context)

    def post(self, request, *args, **kwargs):
        form = EditResultForm(request.POST)
        context = {'form': form, 'page_title': "O'quvchi natijasini tahrirlash"}
        if form.is_valid():
            try:
                ...
                messages.warning(request, "Natijani yangilab boʻlmadi")
        else:
            messages.warning(request, "Natijani yangilab boʻlmadi")
        return render(request, "staff_template/edit_student_result.html", context)
